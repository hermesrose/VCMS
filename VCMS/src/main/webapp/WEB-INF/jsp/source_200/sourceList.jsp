            ...
            <div class="row m-t-lg">
                <div class="col-md-12">
                    <div class="form-group">
                            <form action="/VCMS/SOURCE_200/sourceList.do"  id="searchForm" name="searchForm"  method="POST">
                                <div class=SearchBox>
                                    <table class=Layout>
                                        <tbody>
                                            <tr class="First">                                      
                          ...
                          <button  class="btn btn-primary" id="btnNavbarSearch" onclick="location.href='/VCMS/SOURCE_200/sourceWrite.do'" >申請する</button>
                          </div>
									...
									<table cellspacing="0" cellpadding="0" rules="all" border="0"
										...
										<c:forEach items="${LISTSELECT}" var="list" varStatus="status">
											<tr>
												...
												<td style="text-align: center;"><a href="javascript:void(0);" onclick="location.href='/VCMS/SOURCE_200/sourceView.do?scmAppNo=${list.SCM_APP_NO}'" > ${list.SCM_WORK_NM} </a></td>
												...
												<c:choose>
                                                   <c:when test="${list.SCM_MOVN_STAT eq '21'}">&nbsp;差し戻し </c:when>	
                                                   <c:otherwise>${list.SCM_MOVN_STAT}</c:otherwise>											
 												</c:choose>	
 												</td>
											</tr>	
											</c:forEach>											
							...
							<div align="center">
								<nav aria-label="Customzing the active number">
									<ul class="pagination justify-content-right" id="main-form-hide-pageing">
										<li class="page-item"><a class="page-link" href="/VCMS/SOURCE_200/sourceList.do?row=${startPage-10}&&scmDateFrom=${scmDateFrom}&&scmDateTo=${scmDateTo}&&scmChContent=${scmChContent}&&scmAppNm=${scmAppNm}&&scmRefFrom=${scmRefFrom}&&scmRefTo=${scmRefTo}&&scmConNm=${scmConNm}&&scmWorkNm=${scmWorkNm}&&scmMovnStat=${scmMovnStat}">以前</a></li>
										<c:forEach begin="${startPage}" end="${endPage}" var="g">	
											<li class="page-item"><a class="page-link"   href="/VCMS/SOURCE_200/sourceList.do?row=${g}&&scmDateFrom=${scmDateFrom}&&scmDateTo=${scmDateTo}&&scmChContent=${scmChContent}&&scmAppNm=${scmAppNm}&&scmRefFrom=${scmRefFrom}&&scmRefTo=${scmRefTo}&&scmConNm=${scmConNm}&&scmWorkNm=${scmWorkNm}&&scmMovnStat=${scmMovnStat}">${g}</a></li>			
										</c:forEach>
										<li class="page-item"><a class="page-link" href="/VCMS/SOURCE_200/sourceList.do?row=${startPage+10}&&scmDateFrom=${scmDateFrom}&&scmDateTo=${scmDateTo}&&scmChContent=${scmChContent}&&scmAppNm=${scmAppNm}&&scmRefFrom=${scmRefFrom}&&scmRefTo=${scmRefTo}&&scmConNm=${scmConNm}&&scmWorkNm=${scmWorkNm}&&scmMovnStat=${scmMovnStat}">次</a></li>
									</ul>
								</nav>
							</div>
						</div>
