...
function comentCheck(){   
	   ...
	   var scmStuCont = document.getElementById("scmStuCont").value;
     
       if(scmStuCont.length >50 || scmStuCont.length ==0){
  		  alert("検討処理内訳の文字範囲は1~50です。");
  		 document.getElementById("scmStuCont").focus();
  		 return false;
  		 
      }
             
    	 var form = document.sourceComent;
		  alert("検討処理しました。")
			form.submit();	
 }	  
	...
	<section class="card card-blue mb-3">
                                            ...
                                            <c:forEach items="${sourceView}" var="view" varStatus="status">
                                                ...
                                                <tr>
                                                 ...
                                                 <tr>
                                                    ...
                                                    <td colspan="3">
                                                        <textarea rows="8" cols="8" id=""  name="" class="form-control tableDetail-view" style="padding-top:10px;  background-color:#ffffff;"  maxlength="500" readonly="readonly">${view.SCM_REMARK}</textarea> 
                                                    </td>                                                                                     
                                                </tr>  
                                                 </c:forEach>  
                                            </tbody>
					...
					<div class="form-group">
						<div style="text-align: right">						
											...
											<div class="row m-t-lg">
												<div class="col-md-12">
													<section class="card card-blue mb-3">
														...
														<div class="card-block">
															<form id="sourceComent" name="sourceComent"  action="/VCMS/ADMIN/sourceComentRun.do"   method="POST">       
																<div class="form-group">
																	<div class="tableSideOpen">
																		<table cellspacing="0" cellpadding="0" rules="all"
																			...
																			<c:forEach items="${sourceView}" var="view" varStatus="status">
																				<tr>
																					<th><label class="content-middle">申請番号</label></th>
																					<td><input type="text" id="scmAppNo" name="scmAppNo" class="form-control tableDetail-view" style="background-color: #ffffff;" readonly="readonly" value="${view.SCM_APP_NO}"></td>
																					<th><label class="content-middle">進行状態</label></th>
																					<td><select class="form-control"   id="scmMovnStat"  name="scmMovnStat" style="height: 34px;">
																							<option value="20" <c:if test="${view.SCM_MOVN_STAT eq '申請中'}">selected</c:if>>申請中</option>																					
																							<option value="21" <c:if test="${view.SCM_MOVN_STAT eq '差し戻し'}">selected</c:if>>差し戻し</option>
																							<option value="22" <c:if test="${view.SCM_MOVN_STAT eq '補完要請'}">selected</c:if>>補完要請</option>
																							<option value="23" <c:if test="${view.SCM_MOVN_STAT eq '完了'}">selected</c:if>>完了</option>
																					</select></td>
																				</tr>
																				<tr>
																					<th><label class="content-middle">検討処理内訳</label></th>
																					<td><input type="text" id="scmStuCont" name="scmStuCont" class="form-control" maxlength="100" value="${view.SCM_STU_CONT}"></td>
																				</tr>
																			</tbody>
																			</c:forEach>
																		</table>
							...
							<!-- 상태변경  끝-->
							<button type="button" class="btn btn-secondary btn-sm" onclick="location.href='/VCMS/ADMIN/sourceAdminList.do'">目録移動</button>
						</div>
					</div>
				</div>
			</div>
